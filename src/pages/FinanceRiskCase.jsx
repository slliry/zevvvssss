...
export default function FinanceRiskCase() {
    const { t } = useTranslation();

    const solutionItems = t('financeRiskCase.solutionItems', { returnObjects: true }) || [];
    const resultsItems = t('financeRiskCase.resultsItems', { returnObjects: true }) || [];

    return (
        <div className="bg-[#F7FFFE]">
            <SEO
                title={t('financeRiskCase.title') + ' - Zeus GRC'}
                description={t('financeRiskCase.subtitle')}
                url="/consulting/cases/finance-risk"
            />
                ...
                <div className="relative mx-auto w-full max-w-[1392px] px-6 lg:px-16">
                    <motion.div initial={{ opacity: 0, y: 40 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.7 }} className="max-w-4xl">
                        <p className="mb-4 text-sm font-medium uppercase tracking-widest text-[#004AAD]">
                            {t('financeRiskCase.badge')}
                        </p>
                        <h1 style={{ fontFamily: 'Barlow, Inter, sans-serif', fontWeight: 300, fontSize: '42px', lineHeight: '52px', color: '#1A1A1A' }}>
                            {t('financeRiskCase.title')}
                        </h1>
                        <p className="mt-6" style={{ fontFamily: 'Helvetica, Inter, sans-serif', fontSize: '18px', lineHeight: '29px', color: '#363D42', maxWidth: '760px' }}>
                            {t('financeRiskCase.subtitle')}
                        </p>
                ...
                <div className="mx-auto max-w-[1392px] px-6 lg:px-16">
                    <motion.div initial={{ opacity: 0, y: 20 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true }} transition={{ duration: 0.5 }} className="mb-10 text-center">
                        <h2 style={{ fontFamily: 'Barlow, Inter, sans-serif', fontWeight: 300, fontSize: '36px', lineHeight: '43px', color: '#1A1A1A' }}>
                            {t('financeRiskCase.problemTitle')}
                        </h2>
                    </motion.div>
                    <motion.p initial={{ opacity: 0, y: 20 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true }} transition={{ duration: 0.6 }} style={{ fontFamily: 'Helvetica, Inter, sans-serif', fontSize: '18px', lineHeight: '29px', color: '#363D42' }}>
                        {t('financeRiskCase.problemText')}
                    </motion.p>
                </div>
            </section>
            ...
            <section className="bg-[#F5FCFF] py-16">
                <div className="mx-auto max-w-[1392px] px-6 lg:px-16">
                    <motion.div initial={{ opacity: 0, y: 20 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true }} transition={{ duration: 0.5 }} className="mb-10 text-center">
                        <h2 style={{ fontFamily: 'Barlow, Inter, sans-serif', fontWeight: 300, fontSize: '36px', lineHeight: '43px', color: '#1A1A1A' }}>
                            {t('financeRiskCase.solutionTitle')}
                        </h2>
                    </motion.div>
            ...
            <section className="bg-white py-16">
                <div className="mx-auto max-w-[1392px] px-6 lg:px-16">
                    <motion.div initial={{ opacity: 0, y: 20 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true }} transition={{ duration: 0.5 }} className="mb-10 text-center">
                        <h2 style={{ fontFamily: 'Barlow, Inter, sans-serif', fontWeight: 300, fontSize: '36px', lineHeight: '43px', color: '#1A1A1A' }}>
                            {t('financeRiskCase.resultsTitle')}
                        </h2>
                    </motion.div>
                    <div className="grid grid-cols-1 gap-x-8 gap-y-4 md:grid-cols-2">
                        {Array.isArray(resultsItems) && resultsItems.map((item, i) => (
                            <motion.div key={i} initial={{ opacity: 0, x: -20 }} whileInView={{ opacity: 1, x: 0 }} viewport={{ once: true }} transition={{ duration: 0.4, delay: i * 0.06 }} className="flex items-start gap-3">
                                <CheckCircle2 className="mt-1 shrink-0 text-[#004AAD]" size={20} />
                                <span style={{ fontFamily: 'Helvetica, Inter, sans-serif', fontSize: '18px', lineHeight: '29px', color: '#363D42' }}>{item}</span>
                            </motion.div>
