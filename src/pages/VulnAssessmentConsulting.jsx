...
export default function VulnAssessmentConsulting() {
    const { t } = useTranslation();

    const valueItems = t('vulnAssessment.value', { returnObjects: true }) || [];
    const offerItems = t('vulnAssessment.offer', { returnObjects: true }) || [];

    return (
        <div className="bg-[#F7FFFE]">
            <SEO
                title={t('vulnAssessment.title') + ' - Zeus GRC'}
                description={t('vulnAssessment.subtitle')}
                url="/consulting/vuln-assessment"
            />
                    ...
                    <motion.div initial={{ opacity: 0, y: 40 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.7 }} className="max-w-3xl">
                        ...
                        <div className="mt-10 flex flex-wrap gap-4">
                            <Link to="/request" className="inline-flex items-center gap-2 rounded-full bg-[#004AAD] px-8 py-4 text-white transition-all hover:bg-[#003580]" style={{ fontFamily: 'Space Grotesk, sans-serif', fontWeight: 500, fontSize: '16px' }}>
                                {t('vulnAssessment.ctaMain')} <ArrowRight size={18} />
                            </Link>
            ...
            <section className="bg-white py-16">
                <div className="mx-auto max-w-[1392px] px-6 lg:px-16">
                    <motion.div initial={{ opacity: 0, y: 30 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true }} transition={{ duration: 0.6 }} className="flex flex-col gap-6" style={{ fontFamily: 'Helvetica, Inter, sans-serif', fontSize: '18px', lineHeight: '29px', color: '#363D42' }}>
                        <p>{t('vulnAssessment.desc1')}</p>
                        <p>{t('vulnAssessment.desc2')}</p>
                    </motion.div>
                </div>
            </section>
